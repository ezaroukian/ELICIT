...
def makeResFile(filename):
        ...
        #headers
        foutw.writerow(["worker", "language", "age", "gender", "education", "block", "qType", "cond", "scenario",   "who","whoCorrect", "what","whatCorrect", "where","whereCorrect", "month","monthCorrect", "day","dayCorrect", "time","timeCorrect", "ampm","ampmCorrect", "responseTime", "comments", "SUS",  "polarity", "response", "mental", "physical", "temp", "success", "perf", "stress", "preference"])#add correct/incorrect for each?
        ...
        for r in finr:
                ...
                if len(r)>1 and r[0][0] != "#":
                        ...
                        if r[7] in ["Language", "Age", "Gender", "Education"]:
                        ...
                        #Response data
                            ...
                            exec('{VAR} = {VALUE}'.format(VAR = r[7], VALUE = repr(r[8])))
                        if r[5]!="inst" and (r[7] == "ampm" or r[7] == "_REACTION_TIME_"): #"comments" or r[7] == "preference"
                               writeRow=True


                        itemLabel = r[5].split(".")
                        if len(itemLabel)>1:
                            if len(itemLabel)==2:#end.a/b
                                block="NA"
                                cond=itemLabel[1]
                                scen="NA"
                                qType="questionnaire"
...
##                                            ampmCorrect = (ampm=="AM")
                        ...
                        row = [worker, Language, Age, Gender, Education, block, qType, cond, scen,   who,whoCorrect, what,whatCorrect, where,whereCorrect, month,monthCorrect, day,dayCorrect, time,timeCorrect, ampm,ampmCorrect, _REACTION_TIME_, comments, SUS, polarity, response, mental, physical, temp, success, perf, stress, preference]#add comment somewhere
